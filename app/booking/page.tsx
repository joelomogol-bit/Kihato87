...
export default function Booking() {
  ...
  return (
    <main className="mx-auto max-w-md px-6 py-16">
      <motion.div initial={{ opacity: 0, y: 15 }} animate={{ opacity: 1, y: 0 }} className="rounded-xl border border-zinc-900 bg-[#0a0a0a] p-6 shadow-2xl">
        ...
        {isSubmitted ? (
          <div className="py-12 text-center space-y-4">
            <div className="mx-auto h-12 w-12 bg-emerald-500/10 rounded-full flex items-center justify-center text-emerald-400"><CheckCircle className="h-6 w-6 animate-bounce" /></div>
          ...
          <form onSubmit={handleSubmit} className="space-y-4">
            ...
            <div>
              ...
              <textarea rows={3} placeholder="Describe requirements (e.g. house call)..." value={formData.notes} onChange={(e) => setFormData({...formData, notes: e.target.value})} className="w-full bg-zinc-950 border border-zinc-800 rounded px-3 py-2 text-white text-sm resize-none focus:outline-none focus:border-[#D4AF37]" />
            </div>
            <button type="submit" className="w-full bg-[#D4AF37] hover:bg-[#c29f2f] text-black font-bold py-3 rounded text-sm transition">Confirm Configuration</button>
          </form>
        )}
      </motion.div>
    </main>
